...
const CUSTOM_FOLDER = path.join('tf', 'custom');
const GAMEINFO_FILE = path.join('tf', 'gameinfo.txt');
...
let tools = [
  {
    id: 'hlmvplusplus',
    name: 'HLMV++',
    logo: 'hlmvplusplus.png',
    executable: () => 'hlmvplusplus.exe',
    requiredFiles: [
      'hlmvplusplus.exe',
    ],
  },
...
function updateGameInfoLoadOrder(contents, orderedVpks) {
  ...
  const newLines = orderedVpks.map(vpk => (
    `${indent}"game+mod+custom_mod"\t"tf/custom/${normalizeVpkName(vpk)}"`
  ));
...
function main(context) {
  ...
  context.registerLoadOrder({
    ...
    usageInstructions: 'Arrange the .vpk archives in the order they should be loaded. Mods/vpks higher in the load order will overwrite mods/vpks lower in the load order. Entries are written into gameinfo.txt as custom search paths above the custom/* wildcard entry.',
